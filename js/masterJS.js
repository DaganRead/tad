  ...
  var members = {
    ...
    */
    ...
    scrollBy: function(left, top, animate) {
      ...
      $('#video-container').css('top', startTop + (top || 0));
      $('iframe').css('top', startTop + (top || 0));
...
//modified to limit scrolling when moving in y direction
    ...
    */
    ...
    __startDeceleration: function(timeStamp) {
      ...
      if (self.options.paging) {
        ...
        if(window.currentScrollHeight !=0 ){
          self.__maxDecelerationScrollTop = $('.cn-content').height();
        }else{
          self.__maxDecelerationScrollTop = window.currentScrollHeight;
        }
    ...
    __stepThroughDeceleration: function(render) {
      ...
      //set iframe and video container to follow scrollerContainer
      $('#video-container').css('top', scrollTop);
      $('iframe').css('top', scrollTop);
      ...
      if (self.options.bouncing) {
        ...
        if (scrollLeft < self.__minDecelerationScrollLeft) {
          ...
          if(!window.cellTest){
            ...
            alert(scrollLeft + 'L' + cellTest);
          }
        } else if (scrollLeft > self.__maxDecelerationScrollLeft) {
          scrollOutsideX = self.__maxDecelerationScrollLeft - scrollLeft;
          if(!window.cellTest){
            window.cellTest = true;
            alert(scrollLeft + 'R' + cellTest);
          }
        }
...
function scrollReset(){
  ...
  scroller.scrollTo(cellWidth, 0, false);
  window.currentScrollHeight = 0;
}
...
function btn_scrollDown(){
  ...
  scroller.scrollBy(left1, (screenHeight-((screenHeight/100)*25)), true);
  window.pageCounter ++;
...
;( function( window ) {
  ...
  DotNav.prototype._init = function() {
    ...
    var dots = [].slice.call( this.nav.querySelectorAll( 'li' ) ), current = 1, self = this;
    ...
    dots.forEach( function( dot, idx ) {
      dot.addEventListener( 'click', function( ev ) {
        ...
        if(current === 0 && idx === 1){
            var cellWidth = $('.cell').width();
            if(cellTest){scroller.scrollBy(cellWidth, 0, true);}
            window.currentScrollHeight = 0;
                  if (pageCurrent === 0) {
                document.getElementById('btn_scrollUp').disabled = false;
                document.getElementById('btn_scrollDown').disabled = false;
                pageCurrent++;
                videoBlur();
              }else if(pageCurrent === 1){
                document.getElementById('btn_scrollUp').disabled = true;
                document.getElementById('btn_scrollDown').disabled = true;
                pageCurrent++;
              }
        }else if (current === 1 && idx === 2){
            var cellWidth = $('.cell').width();
            if(cellTest){scroller.scrollBy(cellWidth, 0, true);}
            window.currentScrollHeight = 0;
                  if (pageCurrent === 0) {
        ...
        if(current === 2 && idx === 0){
          var cellWidth = $('.cell').width();
          if(cellTest){scroller.scrollBy(-(2*cellWidth), 0, true);}
          window.currentScrollHeight = 0;
          pageCurrent=0;
        }else if(current === 0 && idx === 2){
          var cellWidth = $('.cell').width();
          if(cellTest){scroller.scrollBy(2*cellWidth, 0, true);}
          window.currentScrollHeight = 0;
          pageCurrent=2;
 ...
 ;(function($, undefined) {
  ...
  $.fn.SuperBox = function(options) {
    ...
    var sbIsIconShown = false,
      ...
      sbImg = $('<div class="superbox-current-img">'+glossaryTerms[window.regX]+glossaryDescriptionOld+'<br/></div>').append('<a href="#slide-1-2" onclick="glossaryScrollTo('+glossaryTerms[window.regX]+')">View in glossary</a>'),
    ...
    var createSuperboxShow = function(elem){
       ...
       */
        ...
        moveToTop = function(){
          ...
          var left1 = window.pageXOffset || document.documentElement.scrollLeft;
          scroller.scrollBy(left1, (target.offset().top - elem.width() -navbarHeight), true);
        },
...
function glossaryMakeLink(){
  for(var i=0; i<glossaryTerms.length; i++){
    document.getElementById("slide-1-1").innerHTML = replaceTextAll(glossaryTerms[i], document.getElementById("slide-1-1").innerHTML);
  }
  for(var i=0; i<window.regX; i++){
    $('.MyGallery'+i).SuperBox();
  }
}
function glossaryBreakLink( btn ){
...
function glossaryScrollTo(GlossaryTermID){
  scrollReset();
  setTimeout(function(){
    var target = document.getElementById(GlossaryTermID);
    var navbarHeight = $('#navbar').height();
    var left1 = window.pageXOffset || document.documentElement.scrollLeft;
    scroller.scrollBy(left1, (target.offsetTop-navbarHeight), true);
  },600);
}